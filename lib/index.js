...
const POWERSHELL = process.platform === 'win32' ? 'C:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe' : null
...
/** Write one UTF-8 text as a GBK temp file (the forum is GBK — UTF-8 bytes would garble Chinese). */
async function writeGbkFile(sub, text, prefix) {
  const file = join(tmpdir(), 'd4y_' + prefix + '_' + Date.now() + '_' + Math.random().toString(36).slice(2, 8) + '.txt')
  if (process.platform === 'win32') {
    // Windows: PowerShell 5.1 with codepage 936. Single-quote paths are escaped.
    const safe = String(file).replace(/'/g, "''")
    const script = "[Console]::InputEncoding=[Text.Encoding]::UTF8; $t=[Console]::In.ReadToEnd(); $enc=[Text.Encoding]::GetEncoding(936); [IO.File]::WriteAllBytes('" + safe + "', $enc.GetBytes($t))"
    await runPwsh(sub, script, String(text))
  } else {
    // Linux/macOS: system iconv (glibc / macOS ship it) writes the GBK file directly.
    const handle = sub.spawn({
      argv: ['iconv', '-f', 'UTF-8', '-t', 'GBK', '-o', file],
      stdio: { stdin: { data: String(text) }, stdout: 'ignore', stderr: 'pipe' },
      graceMs: 15000,
    })
    let errTail = ''
    handle.stderr.on('data', (c) => { try { errTail = (errTail + String(c)).slice(-300) } catch (e) { /* ignore */ } })
    const r = await handle.done
    if (r.exitCode !== 0) {
      throw new Error('GBK 转码失败(需要 iconv):' + (errTail || 'exit ' + r.exitCode))
    }
  }
  return file
}
...
/**
 * Stage one post in the site's own encoding: subject/message become GBK temp
 * files (PowerShell on Windows, iconv elsewhere), image payloads are written
 * straight from Node as real binary files (no size limit, no encoding hop).
 */
async function stagePost(sub, subject, message, images) {
  const [subjectFile, messageFile] = await Promise.all([
    subject ? writeGbkFile(sub, subject, 'sub') : Promise.resolve(''),
    message ? writeGbkFile(sub, message, 'msg') : Promise.resolve(''),
  ])
  const imagesOut = []
  ...
  return { subject: subjectFile, message: messageFile, images: imagesOut }
}
...
export { addslashes, apply, browse, checkLoggedIn, clean, doLogin, doPost, extractPostError, fetchHtml, md5hex, parseBlacklist, parseForum, parseHome, parsePages, parseThread, postOptions, runPwsh, sanitizeContent, stagePost, writeGbkFile }
